...
@router.post("/cluster")
async def trigger_clustering(db=Depends(get_db)):
...
async def get_incident(incident_id: str, db=Depends(get_db)):
    ...
    for tid in ticket_ids:
        try:
            t = await db.tickets.find_one({"_id": ObjectId(tid)}, {"title": 1, "status": 1, "priority": 1, "userId": 1})
            if t:
                t["_id"] = str(t["_id"])
                tickets.append(t)
        except Exception:
...
async def list_kb_articles(db=Depends(get_db)):
    articles = []
...
async def get_kb_article(article_id: str, db=Depends(get_db)):
    try:
...
async def generate_kb_article(body: dict, db=Depends(get_db)):
    ...
    if not article:
        raise HTTPException(status_code=422, detail="Ticket has no resolution steps to generate KB article from")
    article["_id"] = str(article["_id"])
    return article


@router.post("/kb/articles/{article_id}/feedback")
async def kb_article_feedback(article_id: str, body: dict, db=Depends(get_db)):
